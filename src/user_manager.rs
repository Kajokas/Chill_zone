...
async fn log_in(login: Json<LogIn>, mut db: Connection<Db>, cookies: &CookieJar<'_>) -> (Status, String) {
    ...
    match results {
        ...
        Err(e) => {
            eprintln!("Error: {}", e);
            (Status::InternalServerError, String::from("Something went wrong"))
        }
        Ok(user) => {
            let parsed_hash = PasswordHash::new(&user.psw).unwrap();

            if Argon2::default().verify_password(login.psw.as_bytes(), &parsed_hash).is_err(){
...
async fn sign_up(mut user: Json<User>, mut db: Connection<Db>, cookies: &CookieJar<'_>) -> (Status, SignUpResponse) {
    if user.validate().is_err() {
        return (Status::BadRequest, SignUpResponse::ErrorMessage("Invalid data!".to_string()));
    }

    let salt = SaltString::generate(&mut OsRng);
    let argon2 = Argon2::default();

    let hashed_psw = argon2.hash_password(user.psw.as_bytes(), &salt).unwrap().to_string();
    ...
    match results {
        Err(Error::Database(e)) => {
            if e.is_unique_violation() {
                (Status::BadRequest, SignUpResponse::ErrorMessage(String::from("User already exist!")))
            } else {
                eprintln!("Error: {}", e);
                (Status::InternalServerError, SignUpResponse::ErrorMessage(String::from("Something went wrong")))
            }
        }
        Err(e) => {
            eprintln!("Error: {}", e);
